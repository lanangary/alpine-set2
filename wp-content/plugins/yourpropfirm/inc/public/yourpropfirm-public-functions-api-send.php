...
function yourpropfirm_send_api_on_order_status_change($order_id, $old_status, $new_status, $order) {
    ...
    // Check if API has already been sent for this order - exit early
    if ($ypf_connection_completed == 1) {
        return;
    }
    ...
    if ($new_status == 'completed' && $old_status != 'completed') {
        ...
        if (false === get_transient('send_api_lock_' . $order_id)) {
            // Set transient to prevent duplicate API calls within 60 seconds
            set_transient('send_api_lock_' . $order_id, true, 60);
            ...
            foreach ($order->get_items() as $item_id => $item) {
                ...
                if ((!empty($program_id) || !empty($competition_id)) && !$first_product) {
                    ...
                    // Check if response is 201, 400, 401, or 500 - lock to prevent retry
                    if (in_array($http_status, [201, 400, 401, 500])) {
                        // Lock immediately for these status codes using both methods
                        ...
                        $order->save(); // Save immediately after setting lock

                        // Add order note about the lock
                        $lock_note = "--YourPropFirm Status--\n";
                        $lock_note .= "API request completed with HTTP status: " . $http_status . "\n";
                        $lock_note .= "YourPropFirm Completed: Locked (1)\n";
                        $lock_note .= "This order will not trigger duplicate API calls.\n";
                        $lock_note .= "--End Status--\n";
                        $order->add_order_note($lock_note);
                        $order->save(); // Save again with note

                        // If not success (201), exit early
                        if ($http_status != 201) {
                            delete_transient('send_api_lock_' . $order_id);
                            return; // Exit without processing additional quantities
                        }
                    }
            ...
            // Note: Transient will auto-expire after 60 seconds
            // Lock is already set after first API response (201, 400, 401, or 500)
        }
    }
}
