...
function yourpropfirm_post_meta_on_order_creation($order_id) {
    ...
    // Only set connection_completed to 0 if it doesn't exist yet (prevent overwriting completed status)
    $existing_status = get_post_meta($order_id, '_yourpropfirm_connection_completed', true);
    if ($existing_status === '' || $existing_status === false) {
        // Meta doesn't exist, initialize to 0
        update_post_meta($order_id, '_yourpropfirm_connection_completed', 0);
    }
    // If $existing_status is '0' or '1', don't change it
}
